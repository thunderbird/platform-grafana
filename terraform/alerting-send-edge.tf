...
# alerting-appointment-edge.tf, which is NORMATIVE here and not restated. The short
# version: CloudFront and Route53 publish in us-east-1 only, so every query overrides
# region per-model rather than adding a second datasource; severity=warning routes via
# the "root" notification policy to the low-urgency PagerDuty contact point (Slack, no
# page) because tb-dev is a dev cluster; and groups are count-gated on their id/ARN
# variable so an unpinned id means the rule is ABSENT, not a query on a dead dimension.
#
# ---------------------------------------------------------------------------
